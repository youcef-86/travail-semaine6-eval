...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : ven. 12 fév. 2021 à 11:51
...
-- Procédures
--
DROP PROCEDURE IF EXISTS `ajoutCategorie`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ajoutCategorie` (IN `p_cat_name` VARCHAR(50), IN `p_cat_parent_id` INT(10))  BEGIN
...
DROP PROCEDURE IF EXISTS `calculMontantFacture`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `calculMontantFacture` (IN `p_order_id` INT(10), INOUT `p_total` INT(10))  BEGIN
...
DROP PROCEDURE IF EXISTS `calculTotal`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `calculTotal` (`p_produit_id` VARCHAR(50), OUT `p_total` INT(10))  BEGIN
...
DROP PROCEDURE IF EXISTS `calculTotalProduit`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `calculTotalProduit` (`p_produit_id` VARCHAR(50), OUT `p_nom_produit` VARCHAR(50), OUT `p_total` INT(10))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CA_Supplier` (`p_sup_adress` VARCHAR(50), `p_sup_city` VARCHAR(50), `p_sup_contat` VARCHAR(50), `p_sup_id` INT(10), `p_sup_mail` VARCHAR(50), `p_sup_name` VARCHAR(50), `p_sup_phone` VARCHAR(50), `p_sup_zipcode` VARCHAR(50))  BEGIN
...
DROP PROCEDURE IF EXISTS `listeClientParVille`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `listeClientParVille` (IN `p_ville` VARCHAR(50))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Lst_Rush_Orders` (IN `p_commande_urgente` VARCHAR(50))  BEGIN
   SELECT ord_id, ord_order_date, ord_reception_date, ord_ship_date, ord_status 
   FROM orders
   WHERE ord_status = p_commande_urgente;
END$$

DROP PROCEDURE IF EXISTS `Lst_Rush_Orders_total`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Lst_Rush_Orders_total` (IN `p_libelle` VARCHAR(50), OUT `p_total` INT)  BEGIN 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `verifClient` (IN `p_nom` VARCHAR(50), IN `p_prenom` VARCHAR(50), IN `p_adresse` VARCHAR(150), IN `p_cp` VARCHAR(5), IN `p_ville` VARCHAR(50), IN `p_countries_id` CHAR(2), IN `p_mail` VARCHAR(255), IN `p_phone` VARCHAR(10), IN `p_password` VARCHAR(60))  BEGIN
...
INSERT INTO `commander_articles` (`com_id`, `qte`, `date`, `codart`) VALUES
(1, 2, '2021-02-12', 19),
(3, 5, '2021-02-12', 8);
...
(101, 'Despates', 'Ondine', '47 avenue Patrick Sébastien', '59280', 'Armentières', 'FR', 'o.despates@barilla.fr', '0102030405', 'az3rty', '2021-02-09 14:22:15', NULL);
...
DELIMITER $$
CREATE TRIGGER `after_products_update` AFTER UPDATE ON `products` FOR EACH ROW IF new.pro_stock < new.pro_stock_alert THEN 
...
(6, 'youcef', 'Amiens', 'FR', '30 rue poulainville', '80000', 'carlito', '788659944', 'marionafpa@gmail.com');
...
DROP VIEW IF EXISTS `v_ventes_zoom`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_ventes_zoom`  AS  select `products`.`pro_id` AS `pro_id`,`products`.`pro_cat_id` AS `pro_cat_id`,`products`.`pro_price` AS `pro_price`,`products`.`pro_ref` AS `pro_ref`,`products`.`pro_ean` AS `pro_ean`,`products`.`pro_stock` AS `pro_stock`,`products`.`pro_stock_alert` AS `pro_stock_alert`,`products`.`pro_color` AS `pro_color`,`products`.`pro_name` AS `pro_name`,`products`.`pro_desc` AS `pro_desc`,`products`.`pro_publish` AS `pro_publish`,`products`.`pro_sup_id` AS `pro_sup_id`,`products`.`pro_add_date` AS `pro_add_date`,`products`.`pro_update_date` AS `pro_update_date`,`products`.`pro_picture` AS `pro_picture`,`orders_details`.`ode_id` AS `ode_id`,`orders_details`.`ode_unit_price` AS `ode_unit_price`,`orders_details`.`ode_discount` AS `ode_discount`,`orders_details`.`ode_quantity` AS `ode_quantity`,`orders_details`.`ode_ord_id` AS `ode_ord_id`,`orders_details`.`ode_pro_id` AS `ode_pro_id` from (`products` join `orders_details` on((`products`.`pro_id` = `orders_details`.`ode_pro_id`))) where (`products`.`pro_ref` = 'ZOOM') ;
...
ALTER TABLE `commander_articles`
  ADD CONSTRAINT `codarticle` FOREIGN KEY (`codart`) REFERENCES `products` (`pro_id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
